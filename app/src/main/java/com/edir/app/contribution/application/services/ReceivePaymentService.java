...
@AllArgsConstructor
@Service
class ReceivePaymentService implements ReceivePaymentUseCase {
    ...
    public void receivePayment(ReceivePaymentCommand command) {
        MemberContribution ledger = memberRepository
            ...
            .orElseThrow(()-> new MemberContributionNotFoundException(command.memberId()));

        Payment payment = Payment.receive(Money.of(command.amount()), command.paymentDate(),
                    new MemberId(command.receipterId()),
                    command.receiptNumber(),
                    command.remark());


        memberContributionService.receivePayment(ledger,payment);
